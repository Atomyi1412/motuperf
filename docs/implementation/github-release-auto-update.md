...
- v0.23.0 起日志与更新提示窗口按内容自适应高度，并限制在所在屏幕工作区；超高正文在窗口内滚动，底部操作保持可见。
- `latest.json` 的可选 `releaseNotes` 是该版本的纯文本条目数组，最多24条、每条最多1000字符。发布脚本从对应版本 CHANGELOG 提取，无匹配或没有条目则阻止发布；GitHub Release 正文同步使用同一段日志。schema 保持1，旧客户端忽略新字段。
- 新客户端直接展示远端待升级版本的条目，缺失/无效说明显示缺省文本，不回填本地旧版说明。客户端必须先升级到v0.23.0才能显示新布局，旧客户端不能仅靠新清单改变界面。
- “稍后提醒”只关闭本次提示，不设定倒计时；启用启动检查时下次启动仍会提示。“跳过此版本”保存版本号，只屏蔽该版本的自动提示；更高版本、手动检查或强制更新不受屏蔽。
- 发布验收分开记录：测试/构建通过、公开资产与清单哈希匹配、旧版发现更新、下载校验和安装器交接、安装后实际运行版本。构建成功不能替代后四项。
- 发布脚本需兼容 Windows PowerShell 5.1；依赖 `$PSScriptRoot` 的默认路径在参数绑定后初始化，不放入 `param()` 默认表达式。回归测试覆盖不传 `ChangelogPath` 的实际发布命令，并在可用的 PowerShell 5.1/7 下执行。
